...
class Croco_School_Articles extends Croco_School_Base {
		...
		?><div <?php echo $this->get_render_attribute_string( 'container' ); ?>>
			<div class="croco-school-guide__inner"><?php
				...
				if ( ! empty( $atricle_thumbnail_id ) ) {
					echo sprintf( '<div class="croco-school-guide__thumbnail">%s</div>', wp_get_attachment_image( $atricle_thumbnail_id, 'full' ) );
				}
				...
				if ( ! empty( $term_data->name ) ) {
					...
					if ( ! empty( $back_button_text ) && $use_back_button ) {
						...
						$back_button_html = sprintf( '<a class="croco-school-guide__back" href="%s"><i class="nc-icon-glyph arrows-1_tail-left"></i>%s</a>', $back_button_url['url'], $back_button_text );
					}
	...
	public function generate_article_list( $term_id = '', $term_name_visible = false ) {
		...
		if ( $use_article_limit ) {
			...
			echo sprintf( '<div class="croco-school-guide__more-articles"><a href="%s"><span>%s</span>%s</a></div>', $term_link, $term_link_text, $more_link_icon_html );
		}
	}
